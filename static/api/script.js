...
let totalPurchased = 0; // STORAGE FOR THE OVERALL TOTAL
let rowCount = 0; // FOR DISPLAYING THE MESSAGE IF THE ROW DATA IS EMPTY
let primaryId = 0; // FOR REMOVING THE ROW, USING THE ID
let editId = null; // FOR EDIT TO DETERMINE WHAT ROW SHOULD THE CHANGES APPLY
...
document.body.addEventListener('click', (e) => {
    if(e.target.matches('.edit-btn')){
        e.preventDefault();
        const id = e.target.closest('tr').dataset.id; // get the id of the item to be edited
        editId = id;
...
edit.addEventListener('click', (e) => {
        ...
        const formattedPrice = totalPurchased.toFixed(2);
...
document.body.addEventListener('click', (e) => {
    if(e.target.matches('.remove-btn')){
        ...
        if (row){ // check if there is a result
            ...
            rowCount -= 1; // decrease the count
            totalPurchased -= parseFloat(tdAmount.value); // minus the total
            const formattedPrice = totalPurchased.toFixed(2);
            total.value = '$'+formattedPrice;
        }
...
async function sendBarcode(barcode){
    try{
        ...
        if (result.success){
            ...
            totalPurchased += totalQty * itemPrice; //for overall total
            // TOTAL PRICE FORMATTED
            const formattedPrice = totalPurchased.toFixed(2); // get the total purchased in 2 decimal place
            // CREATE A TR
            primaryId += 1;
            const tr = document.createElement('tr');
            tr.dataset.id = primaryId; // SET PRIMARY ID FOR REMOVING
            // CREATE TD AND ADD DATA ON EACH 12345
            let subtotal = totalQty != 1 ? totalQty * itemPrice : itemPrice; //get subtotal
            const datas = {
            ...
            for(let i = 0; i < keys.length + 1; i++){
                ...
                if (i === keys.length){ // IF LAST LOOP, CREATE THE BUTTONS
                    ...
                    if (keys[i] === 'subtotal') td.value = subtotal.toFixed(2); // PUT THE VALUE ON TD FOR FUTURE RETRIEVE
